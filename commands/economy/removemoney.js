...
module.exports = {
    ...
    usage: [''],
    examples: [""],
    description: "Permet d'enlever de l'argent à un utilisateur",
    cooldown: 10,
    category: 'economy',
    options: [
        {
    ...
    runInteraction: async (client, interaction, db) => {
        ...
        db.query(`SELECT * FROM economy WHERE guildID = ? AND userID = ?`, [interaction.guild.id, interaction.user.id], async (err, req) => {
            ...
            if (coins > 2000) {
                interaction.reply({ content: `Votre sommes est supérieur à 2000 <:Torasdor:969166509756809216>.`, ephemeral: true })
                ...
                db.query(`SELECT * FROM economy WHERE userID = ?`, [target.id], async (err, req) => {
                    ...
                    } else if (coins > req[0].coins) {

                        interaction.reply({ content: `Vous ne pouvez pas retirer plus d'argent que l'utilisateur ne possède.`, ephemeral: true })
                    } else {

                        db.query(`UPDATE economy SET coins = coins - ? WHERE guildID = ? AND userID = ?`, [coins, interaction.guildId, target.user.id]);

                        const embed = new MessageEmbed()
                            .setDescription(`${interaction.member} a enlevé ${coins} <:Torasdor:969166509756809216> à ${target.user.tag}`)
                            .addField(`** **`, `${target.user.tag} possède désormé ${parseInt(req[0].coins) - coins} <:Torasdor:969166509756809216>`)
